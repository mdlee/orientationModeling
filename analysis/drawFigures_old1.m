...
for analysisIdx = 1:numel(analysisList)
  ...
  switch analysisName
    ...
    case 'descriptiveAdequacyPerceptualOld'
      ...
      if exist('descriptiveAdequacyPerceptualPreSave.mat', 'file')
        ...
        [nSamples, ~] = size(chains.sigma(:));
        count = nan(dp.nStimuli, length(binsC));
        for idx = 1:dp.nStimuli
          pred = randn(nSamples, 1).*chains.sigma(:) + chains.(sprintf('mu_%d', idx))(:);
          resp = dp.response(find(dp.sIdx == idx));
          D = resp(:) - pred(:).';
          D = D(:);
          for i = 1:length(D)
            V = [D(i) D(i)+pi D(i)-pi D(i)+2*pi D(i)-2*pi];
            [~, tmp] = min(abs(V));
            D(i) =V(tmp);
          end
          count(idx, :) = histcounts(D, 'BinEdges', binsE, ...
            'norm', 'prob');
        end

        save descriptiveAdequacyPerceptualPreSave count binsC
      ...
      set(gca, ...
        ...
        'clipping'   , 'off'     , ...
        'fontsize'   , fontSize  );
      xlabel('Stimulus', 'fontsize', fontSize+2);
      ylabel('Posterior Predictive Difference', 'fontsize', fontSize+2);
      moveAxis(gca, [1 1 1 0.9], [0 0.025 0 0]);
      Raxes(gca, 0.01, 0.03);

      for i = 1:dp.nStimuli
        for j = 1:length(binsC)
          if count(i, j) > 0
            plot(i + scale/2*count(i, j)*[-1 1], [binsC(j) binsC(j)], '-', ...
              'color', clr);
          end
        end
      end
  ...
  set(gca, ...
    ...
    case 'descriptiveAdequacyMemory'
      ...
      if exist('descriptiveAdequacyMemoryPreSave.mat', 'file')
        ...
        %         lo = -pi/2; hi = pi+pi/2; eps = 0.1;
        % binsCy = lo:eps:hi;
        % binsEy = lo-eps/2:eps:hi+eps/2;
        ...
        % tmp1 = find(dm.response < 0);
        % tmp2 = find(dm.response > pi);
        % dm.response(tmp1) = dm.response(tmp1) + pi;
        % dm.response(tmp2) = dm.response(tmp2) - pi;

        dm.response = mod(dm.response, pi);

        [~, ~, rIdx] = histcounts(dm.response, 'binedges', binsE);
        
     %   count = nan(numel(binsC), numel(binsCy));
     mn = nan(numel(binsC), 1);
     credInterval = nan(numel(binsC), 2);
        for i = 1:length(binsC)
           match = find(rIdx == i);
          ...
          k = round((binsC(i) - vals) ./ pi);
vals = vals + k .* pi;
          % count(i, :) = histcounts(binsC(i) - vals, 'BinEdges', binsEy, ...
          %   'norm', 'prob');
          mn(i) = mean(vals);
          credInterval(i, :) = prctile(vals, CI);
        end
...
%% OFFCUTS
  ...
  %  plot([0 pi], [0 pi], 'k-');
    ...
    %       'xticklabel' , {'$0$', '$\frac{\pi}{4}$', '$\frac{\pi}{2}$', '$\frac{3\pi}{4}$', '$\pi$'}, ...
    %       'ylim'       , [-pi/2 pi+pi/2]    , ...
    %       'ytick'      , [-pi/2 0 pi/4 pi/2 3*pi/4 pi pi+pi/2]   , ...
    %       'yticklabel' , {'$\frac{\pi}{2}$', '$0$', '$\frac{\pi}{4}$', '$\frac{\pi}{2}$', '$\frac{3\pi}{4}$', '$\pi$', '$\frac{3\pi}{2}$'}, ...
    %       'ticklabelinterpreter', 'latex', ...
    %       'box'        , 'off'     , ...
    %       'tickdir'    , 'out'     , ...
    %       'layer'      , 'top'     , ...
    ...
    % 
    %   for i = 1:length(binsC)
    %     for j = 1:length(binsCy)
    %       if count(i, j) > 0
    %          w = scale*(count(i, j));
    %          rectangle('position', [binsC(i)-w/2 binsCy(j)-w/2 w w], ...
    %             'curvature', [0 0], ...
    %             'facecolor', clr, ...
    %             'edgecolor', clr);
    %         % plot(binsC(i) + scale/2*count(i, j)*[-1 1], [binsCy(j) binsCy(j)], '-', ...
    %         %   'color', clr);
    %       end
    %     end
    %   end
